...
while true
do
	Threads=`ps -A -T | grep $KEYWORD | awk '{if (NR>1) {print $2}}' | sort -r`;
	...
	do
		if [ "$CURRENT" = "$MYSELF" ] ; then continue ; fi
	  	Remotes=`ssh root@$CURRENT ps -A -T | grep popcorn | awk '{if (NR>1) {print $2}}'| sort -r`;
	  	RemoteNum=`ssh root@$CURRENT ps -A -T | grep popcorn | awk '{if (NR>1) {print $2}}' | wc -l `;
		echo $Threads
		echo $CurrentNum
		echo $Remotes
		echo $RemoteNum
